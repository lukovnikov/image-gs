...
class BatchedGSImage(torch.nn.Module):
    def __init__(self, batch_size, num_gaussians, feat_dim, size, dtype, device):
        ...
        self.feat = nn.Parameter(torch.rand(self.batch_size, num_gaussians, self.feat_dim, dtype=self.dtype, device=self.device), requires_grad=True)

    @classmethod
    def from_statedict(cls, statedict):
        batch_size = statedict['xy'].shape[0]
        num_gaussians = statedict['xy'].shape[1]
        feat_dim = statedict['feat'].shape[2]
        size = (statedict['size'][1].item(), statedict['size'][2].item())
